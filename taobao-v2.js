...
// 淘宝登录用户名和密码
const userName = '15350149442';
const pwd = 'll..562230659';
// 抢单开始时间 hh:mm:ss
...
async function firstAction() {
  console.log(`---这是第${qiangDanTimes}次抢单---firstAction`,new Date(Date.now()).toLocaleString());
  // 进入购物车页面
  ...
  await driver.wait(() => {
    console.log('等待抢单时间', targetTime - Date.now(),targetTime - Date.now() <= 0.5)
  ...
  await driver.wait(async () => {
    ...
    if (!curUrl.includes('order/confirm_order.htm') && !curUrl.includes('cart.taobao.com')) {
      console.log('没有跳转到提交订单页面，则认为订单被拦截了，要重新抢单')
      // 重新抢单
      qiangDanTimes++;
      await firstAction();
      return true;
...
async function login(userName, pwd) {
  ...
  try {
    ...
    await driver.findElement(By.css('.icon-qrcode')).click();

    await driver.wait(until.urlContains('https://i.taobao.com/'), waitTime);
